...
const tools = [
    ...
    },
    {
        ...
        description: "Export a Gantry outline into a portable JSON or YAML blueprint that can be reused on another site. Can optionally save under local blueprints/ folder.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Import/apply a Gantry outline blueprint from object, text, or file path. Supports dry-run preview before live apply.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    try {
        switch (name) {
            ...
            case "joomla_gantry5_export_outline_blueprint": {
                ...
                if (!login.success)
                    return { content: [{ type: "text", text: formatResult(login) }], isError: true };
                const result = await joomla.exportGantry5OutlineBlueprint(args?.outline || "default", {
                    theme: args?.theme,
                    format: args?.format,
                    saveToFile: args?.saveToFile,
                    fileName: args?.fileName,
