...
function resolveSpecInput(args) {
    ...
    if (!rawSpec)
        return { error: "spec, specText, or filePath is required" };
...
function normalizeContentSource(raw) {
    if (!raw)
        return undefined;
    const source = asRecord(raw);
    return {
        mode: asString(source.mode, "auto_generate"),
...
function normalizeModule(raw) {
    ...
    return {
        ...
        source: {
            mode: asString(source.mode, "module_blueprint"),
...
function normalizeHomepageParticle(raw) {
    ...
    return {
        ...
        articleBinding: Object.keys(articleBinding).length === 0 ? undefined : {
            mode: asString(articleBinding.mode, "category"),
        ...
        contentBinding: Object.keys(contentBinding).length === 0 ? undefined : {
            mode: asString(contentBinding.mode, "auto_generate"),
...
function validateSpec(spec) {
    ...
    const keys = new Set();
    let homeCount = 0;
    if (!spec.site.siteName)
        errors.push("site.siteName is required");
    if (!spec.site.siteUrl)
        errors.push("site.siteUrl is required");
    if (!spec.site.menuTitle)
        errors.push("site.menuTitle is required");
    if (!spec.site.menuType)
        errors.push("site.menuType is required");
    if (spec.menuItems.length === 0)
        errors.push("At least one menu item is required");
    for (const item of spec.menuItems) {
        if (!item.key)
            errors.push(`A menu item is missing key: ${item.title || "(untitled)"}`);
        if (!item.title)
            errors.push(`Menu item ${item.key || "(missing key)"} is missing title`);
        if (item.key) {
            if (keys.has(item.key))
                errors.push(`Duplicate menu item key: ${item.key}`);
            keys.add(item.key);
        }
        if (item.home)
            homeCount += 1;
        if (item.content?.mode === "pull_url" && !item.content.sourceUrl)
            warnings.push(`Menu item ${item.key} uses pull_url without sourceUrl`);
        if (item.content?.mode === "pull_backend_article" && !item.content.sourceArticleId)
            warnings.push(`Menu item ${item.key} uses pull_backend_article without sourceArticleId`);
    }
    if (homeCount === 0)
        warnings.push("No menu item is marked as home");
    if (homeCount > 1)
        errors.push("Only one menu item should be marked as home");
    if (spec.homepage?.particles) {
        for (const particle of spec.homepage.particles) {
            if (!particle.section)
                warnings.push(`Homepage particle ${particle.title || particle.particleType} is missing section`);
            if (!particle.particleType)
                errors.push(`Homepage particle ${particle.title || "(untitled)"} is missing particleType`);
...
function renderPrompt(spec) {
    ...
    if (spec.site.adminBaseUrl)
        lines.push(`- Admin base URL: ${spec.site.adminBaseUrl}`);
    lines.push(`- Menu title: ${spec.site.menuTitle}`);
    lines.push(`- Menu type: ${spec.site.menuType}`);
    if (spec.site.notes)
        lines.push(`- Site notes: ${spec.site.notes}`);
    lines.push("");
    ...
    for (const item of spec.menuItems) {
        ...
        if (item.content) {
            ...
            if (item.content.guidance)
                lines.push(`  Content guidance: ${item.content.guidance}`);
            if (item.content.sourceUrl)
                lines.push(`  Source URL: ${item.content.sourceUrl}`);
            if (item.content.sourceArticleId)
                lines.push(`  Source backend article ID: ${item.content.sourceArticleId}`);
            if (item.content.sourceArticleTitle)
                lines.push(`  Source backend article title: ${item.content.sourceArticleTitle}`);
            if (item.content.cleanupNotes)
                lines.push(`  Cleanup notes: ${item.content.cleanupNotes}`);
        }
        if (item.artworkNotes)
            lines.push(`  Artwork notes: ${item.artworkNotes}`);
    ...
    if (!spec.moduleCatalog || spec.moduleCatalog.length === 0) {
        ...
        for (const module of spec.moduleCatalog) {
            ...
            if (module.source.blueprintPath)
                lines.push(`  Blueprint path: ${module.source.blueprintPath}`);
            if (module.source.sourceModuleId)
                lines.push(`  Source module ID: ${module.source.sourceModuleId}`);
            if (module.source.sourceUrl)
                lines.push(`  Source URL: ${module.source.sourceUrl}`);
            if (module.source.guidance)
                lines.push(`  Guidance: ${module.source.guidance}`);
    ...
    if (spec.homepage?.artworkNotes)
        lines.push(`- Artwork notes: ${spec.homepage.artworkNotes}`);
    if (spec.homepage?.particles && spec.homepage.particles.length > 0) {
        ...
        for (const particle of spec.homepage.particles) {
            ...
            if (particle.articleBinding) {
                ...
                if (particle.articleBinding.categoryTitle)
                    lines.push(`    Category title: ${particle.articleBinding.categoryTitle}`);
                if (particle.articleBinding.articleTitle)
                    lines.push(`    Article title: ${particle.articleBinding.articleTitle}`);
                if (particle.articleBinding.total)
                    lines.push(`    Article total: ${particle.articleBinding.total}`);
                if (particle.articleBinding.createIfMissing)
                    lines.push(`    Create article if missing: yes`);
                if (particle.articleBinding.generationNotes)
                    lines.push(`    Generation notes: ${particle.articleBinding.generationNotes}`);
            }
            if (particle.contentBinding) {
                lines.push(`    Content binding mode: ${particle.contentBinding.mode}`);
                if (particle.contentBinding.sourceUrl)
                    lines.push(`    Source URL: ${particle.contentBinding.sourceUrl}`);
                if (particle.contentBinding.sourceArticleId)
                    lines.push(`    Source backend article ID: ${particle.contentBinding.sourceArticleId}`);
                if (particle.contentBinding.guidance)
                    lines.push(`    Guidance: ${particle.contentBinding.guidance}`);
            }
            if (particle.notes)
                lines.push(`    Notes: ${particle.notes}`);
        }
    }
    lines.push("");
    ...
    try {
        if (name === "site_build_prompt_scaffold") {
            ...
            if (args.saveToFile === true) {
                ...
                result.data = { ...result.data, filePath };
            }
        ...
        if (name === "site_build_prompt_render") {
            ...
            if (args.savePromptToFile === true) {
                ...
                result.data = { ...result.data, promptFilePath };
            }
            if (args.saveNormalizedSpec === true) {
                ...
                result.data = { ...result.data, normalizedSpecFilePath };
            }
            return { content: [{ type: "text", text: formatResult(result) }], isError: false };
        }
        return {
